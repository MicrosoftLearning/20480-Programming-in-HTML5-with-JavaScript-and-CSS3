...
3.	In **grayscale.js**, in the **grayscaleImage()** function, delete the following code:
    ...
    ```
4.	In **grayscale-worker.js**, add the following code:
    ```javascript
        addEventListener("message", function (event) {
...
9.	Browse to the **[Repository Root]\Allfiles\Mod14\Labfiles\Resources** folder.
...
11.	Verify that you can scroll the page up and down while the image is being processed.
12.	Close Microsoft Edge.
