...
-	Use the jQuery **ajax** method to simplify code that performs remote communications.
...
- Ensure that the **readyState** property of the request object indicates that the response is complete (it should have the value **4**).
- Parse the JSON string in the response into an object, and then assign this object to the **schedule** variable.
...
2.	Near the bottom of the **schedule.htm** file, before the reference to the **schedule.js** script, add a reference to the **jquery.min.js** jQuery JavaScript file that is located in the **scripts** folder. This makes the **jQuery** object available for use in the JavaScript code for the **Schedule** page.
3.	In the **scripts/pages** folder, in the **schedule.js** file, refactor the **downloadSchedule** function to use the jQuery **ajax()** function.
- The ajax options object must have a **type** property of **GET** and a **url** property of **/schedule/list**.
- To provide a callback function that handles the web service response, use the jQuery **done()** function; the response will contain a property called **schedule** that you should assign to the local **schedule** array variable, and then call the **displaySchedule()** function.

>**Note**: An argument containing the parsed JSON response object is passed to the **done** callback, so you do not need to use the **JSON.parse** method to extract the session data.

- To provide a callback function that handles any errors that might occur, use the jQuery **fail()** function; simply display the **Schedule list not available** message in this callback
...
- The data that is passed by using the ajax options object that specifies whether a session has been selected should be a JavaScript object and not a string.
- To implement a callback function that receives the web service response, use the jQuery **done()** function. In this callback, if the **starCount** property in the response object that was passed to the callback is greater than 50, display the **This session is very popular! Be sure to arrive early to get a seat** message.
