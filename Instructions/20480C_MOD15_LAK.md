...
14. In the **webpack.config.js** file, to store the **page** and **webpack** packages into variables, use **require**.
   ...
   ```
15. To configure the **module.export** object with the **entry** object that includes an entry for every JavaScript file that was imported into the html file, use the following code:
   ```javascript
   ...
   ```
16. Add the **output** object to **module.export**, configure the **path** property to indicate where to save the **bundle** files, the **filename** property with the **name** placeholder, and **publicPath** with the **dist** value.
   ```javascript
    ```
   ...
   ```
17. Add the **module** object to **module.export**. Inside the object, add the **rules** array with the **babel-loader** configuration object.
   ```javascript
   ...
   ```
18. To **module.export**, add the **stats**, **devtool**, and **mode** objects.
   ```javascript
